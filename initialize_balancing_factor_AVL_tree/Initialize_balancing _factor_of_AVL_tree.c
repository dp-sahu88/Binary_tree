...
typedef struct tnode{
    int value,height,balancing_factor;
    struct tnode *left;
    struct tnode *right;
}tnode;
...
void initialize_balancing_factor(tnode* root){

       if(root==NULL){
        return;
       }
       root->balancing_factor=getBalance(root);
       initialize_balancing_factor(root->left);
       initialize_balancing_factor(root->right);
...
// this function will print the AVL tree with balancing factor(pre-order traversal)
void pre_order_print_with_bf(tnode* root,int level){
    if (root==NULL){
        print_format(level);
        printf("...\n");
        return;
    ...
    print_format(level);
    printf(" Left\n");
    pre_order_print_with_bf(root->left,level+1);
    print_format(level);
    printf("Right\n");
    pre_order_print_with_bf(root->right,level+1);
}
...
int main(){
   ...
   while (1){
        if (choice==1){
            ...
            system("cls");
            initialize_balancing_factor(root);//this is a recursive function
            printf("\nPrinting the AVL tree with balancing factor \n");
            printf("\nBF : Balancing factor\nL  : level\n");
            pre_order_print_with_bf(root,0);
        }
        else{
            return 0;
        }
    fflush(stdin);//clear buffrer
